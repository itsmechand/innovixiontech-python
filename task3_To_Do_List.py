...
def welcome_msg(name):
    print(f""" **WELCOME $ {name} $ TO TO_DO_LISTS APPLICATION**
        press 1 for UPDATE LIST
        press 2 for TRACK LIST
        press 3 for exit""")
...
def update(name):
    ...
    choice = int(
        input("'1'--> to ADD '2'--> to REMOVE tasks and '3'--> to DELETE List_File: "))
    if choice == 1:
        task = input("Please Enter Task to ADD: ")
        with open(f"{name}.txt", 'a') as f:
            f.write(f"{task} - {date.today()}\n")
    elif choice == 2:
        task = input("Please Enter Task to DELETE: ")
        try:
            # ISSUE HERE
            new_list = ""
            ...
            print("OOPs, I CAN'T REMOVE THIS TASK")
